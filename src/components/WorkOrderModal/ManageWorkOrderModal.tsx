...
const ManageWorkOrderModal: React.FC<ManageWorkOrderModalProps> = ({ open, onClose, workOrder }) => {
  ...
  const handleGenerateInvoice = async () => {
    if (!workOrder?.id || !token) {
      console.error('Missing work order ID or token');
      return;
    }

    if (!invoiceDueDate) {
      alert('Please select a due date for the invoice');
      return;
    }
    ...
    console.log('Work Order ID:', workOrder.id);
    console.log('Due Date:', invoiceDueDate);
    console.log('Notes:', invoiceNotes);
    console.log('Terms:', invoiceTerms);

    const payload = {
      workOrderId: workOrder.id,
      dueDate: new Date(invoiceDueDate).toISOString(),
      notes: invoiceNotes || 'Payment due within 14 days',
      terms: invoiceTerms || 'Net 14'
    };
    ...
    try {
      ...
      // Error will be visible in console, no alert needed
    } finally {
  ...
  return (
    <div className="manage-workorder-modal__overlay" onClick={onClose}>
      ...
      {showGenerateInvoiceModal && (
        <div className="manage-workorder-modal__overlay" onClick={() => setShowGenerateInvoiceModal(false)}>
          <div className="manage-workorder-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '520px' }}>
            ...
            <div className="modal-header" style={{ padding: '16px 20px' }}>
              ...
              <button className="close-btn" onClick={() => setShowGenerateInvoiceModal(false)} title="Close">
                <i className="bx bx-x"></i>
              </button>
            </div>
