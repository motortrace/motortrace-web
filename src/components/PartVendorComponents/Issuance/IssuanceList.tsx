...
const IssuanceList: React.FC = () => {
  ...
  const fetchIssuances = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3000/inventory/issuances');
      ...
      // Normalize response: some endpoints return an array, others return { success: true, data: [...] }
      let items: any[] = [];
      if (Array.isArray(data)) {
        ...
        // Unexpected shape — try to coerce to array if possible or keep empty
