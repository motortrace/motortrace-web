...
export const CostSummaryReport: React.FC<CostSummaryReportProps> = ({ 
  ...
  const [summary, setSummary] = useState({
    totalCost: 0,
    totalIssuances: 0,
    averageCostPerIssuance: 0,
    highestMonthlyCost: 0
  });
  const [monthlyTrend, setMonthlyTrend] = useState<MonthlyCost[]>([]);
  const [categoryBreakdown, setCategoryBreakdown] = useState<CategoryCost[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch cost summary data from API
  useEffect(() => {
    fetchCostSummary();
  }, [dateFrom, dateTo]);
  ...
  const fetchCostSummary = async () => {
    try {
      ...
      console.log('Fetching cost summary from:', endpoint);
      ...
      setError('Failed to load cost summary data. Please try again.');
      setSummary({
        totalCost: 0,
        totalIssuances: 0,
        averageCostPerIssuance: 0,
    ...
    <div className="cost-summary-report">
      <div className="report-header">
        ...
        <div className="header-actions">
          ...
          <button className="refresh-btn" onClick={fetchCostSummary} title="Refresh data">
            ⟳
          </button>
        </div>
      </div>
      ...
      <div className="cost-summary-grid">
        <div className="summary-card total-cost">
          ...
          <div className="card-content">
            <div className="amount">LKR {summary.totalCost.toLocaleString()}</div>
            <div className="label">Total Cost</div>
          </div>
        </div>
        ...
        <div className="summary-card avg-cost">
          ...
          <div className="card-content">
            <div className="amount">LKR {summary.averageCostPerIssuance.toFixed(2)}</div>
            <div className="label">Avg per Issuance</div>
          </div>
        </div>
        ...
        <div className="summary-card highest-month">
          ...
          <div className="card-content">
            <div className="amount">LKR {summary.highestMonthlyCost.toFixed(2)}</div>
            <div className="label">Highest Monthly Cost</div>
          </div>
        </div>
        ...
        <div className="summary-card total-issuances">
          ...
          <div className="card-content">
            <div className="amount">{summary.totalIssuances}</div>
            <div className="label">Total Issuances</div>
          </div>
        </div>
      </div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlyTrend}>
                ...
                <Tooltip 
                  ...
                  formatter={(value) => [`LKR ${value}`, 'Cost']}
                />
                <Line 
                  type="monotone" 
                  dataKey="cost" 
          ...
          <div className="table-body">
            {categoryBreakdown.map((category) => (
              <div key={category.category} className="table-row">
                ...
                <div className="cell">
                  <span className="cost-amount">LKR {category.cost.toLocaleString()}</span>
                </div>
