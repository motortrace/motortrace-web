...
const InspectionRecordsPage = () => {
  ...
  const uniqueInspectors = [...new Set(inspections.map((i: WorkOrderInspection) => 
    i.inspector?.userProfile ? `${i.inspector.userProfile.firstName} ${i.inspector.userProfile.lastName}` : i.inspectorId
  ))];
  ...
  const filteredInspections = inspections.filter((inspection: WorkOrderInspection) => {
    const workOrderNumber = inspection.workOrder?.workOrderNumber || '';
    const inspectorName = inspection.inspector?.userProfile ? 
  ...
  const handleView = (id: string) => {
    const inspection = inspections.find((i: WorkOrderInspection) => i.id === id) || null;
  ...
  const columns: TableColumn<WorkOrderInspection>[] = [
    {
      ...
      render: (_: any, row: WorkOrderInspection) => (
        <strong>{row.workOrder?.workOrderNumber || row.workOrderId}</strong>
      )
    },
    {
      key: 'template',
    ...
    },
    {
      ...
      sortable: true,
      render: (_: any, row: WorkOrderInspection) => {
        const inspector = row.inspector?.userProfile;
        return inspector ? (
    ...
    },
    {
      ...
      render: (isCompleted: any) => getStatusBadge(isCompleted)
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="inspection-records-page">
      ...
      {/* TODO: Add view modal component when needed */}
      {viewModalOpen && (
        <div className="modal-overlay" onClick={() => setViewModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <h3>Inspection Details</h3>
            <p>Work Order: {selectedInspection?.workOrder?.workOrderNumber}</p>
            <p>Template: {selectedInspection?.template?.name || 'No Template'}</p>
            <p>Inspector: {selectedInspection?.inspector?.userProfile ? 
              `${selectedInspection.inspector.userProfile.firstName} ${selectedInspection.inspector.userProfile.lastName}` : 
              'Unknown'}</p>
            <p>Date: {selectedInspection?.date ? new Date(selectedInspection.date).toLocaleDateString() : 'N/A'}</p>
            <p>Status: {selectedInspection?.isCompleted ? 'Completed' : 'In Progress'}</p>
